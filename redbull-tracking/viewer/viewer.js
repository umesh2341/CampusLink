...
export function initViewer(customConfig = {}) {
  ...
  function connect() {
    const url = customConfig.supabaseUrl || document.getElementById('cfgSupabaseUrl')?.value?.trim() || new URLSearchParams(window.location.search).get('url');
    const key = customConfig.supabaseKey || document.getElementById('cfgSupabaseKey')?.value?.trim() || new URLSearchParams(window.location.search).get('key');
    if (!url || !key) return;
  ...
  async function fetchInitial() {
    if (!supabase) return;
    try {
      const { data, error } = await supabase.from('redbull_car_live').select('*').eq('device_label', 'REDBULL_CAR_01').maybeSingle();
      if (!error && data) {
        onTelemetry(data);
        return;
      }
      const fb = await supabase.from('redbull_car_telemetry').select('latitude, longitude, accuracy, altitude, heading, speed, updated_at').eq('device_label', 'REDBULL_CAR_01').maybeSingle();
  ...
  function subscribeRT() {
    if (!supabase) return;
    rtChannel = supabase.channel('redbull-telemetry-live')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'redbull_car_telemetry', filter: 'device_label=eq.REDBULL_CAR_01' }, payload => {
  ...
  function onTelemetry(row) {
    ...
    const { x, y, accuracyRadiusPixels: accR } = transformer.toMapCoordinates(row.latitude, row.longitude, row.accuracy);
    ...
    if (el.landmarkText) el.landmarkText.innerText = loc.description;
    if (el.speedText) el.speedText.innerText = row.speed ? `${(row.speed * 3.6).toFixed(1)} km/h` : '0.0 km/h';
    if (el.timeText) el.timeText.innerText = new Date(row.updated_at || Date.now()).toLocaleTimeString();
  ...
  function setStatus(s) {
    ...
    el.statusBadge.className = 'badge ' + (s === 'LIVE' ? 'badge-green' : s === 'WEAK_GPS' ? 'badge-amber' : s === 'DELAYED' ? 'badge-orange' : 'badge-red');
  }
  ...
  function drawRoute(carPt) {
    ...
    if (el.routeLayer) {
      el.routeLayer.innerHTML = `<path d="${path}" fill="none" stroke="#e11d48" stroke-width="8" stroke-linecap="round" stroke-linejoin="round" stroke-dasharray="14 10" class="nav-route-anim"/><path d="${path}" fill="none" stroke="#fff" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>`;
    }
    if (el.navDetails) {
