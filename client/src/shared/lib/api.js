...
/** Fetch list of all campus student clubs. Hardened with 8s timeout & client fallback. */
export const fetchClubs = async () => {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 8000);
  try {
    const res = await fetch(`${API_BASE}/api/clubs`, { signal: controller.signal });
    clearTimeout(timeoutId);
    if (!res.ok) throw new Error(`HTTP ${res.status}: Failed to fetch clubs`);
    const data = await res.json();
    if (Array.isArray(data) && data.length > 0) {
      return data;
    }
    throw new Error('Empty payload returned from /api/clubs');
  } catch (err) {
    clearTimeout(timeoutId);
    console.warn('[api.js] fetchClubs network fallback triggered:', err.message);
